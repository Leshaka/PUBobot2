...
class BaseRating:
    ...
    def _scale_changes(self, player, r_change, d_change, score):
        p = player.copy()
        r_change = (
            (r_change * self.scale) * self.reduction_scale
            if r_change < 0
            else r_change * self.scale
        )
    ...
    async def set_rating(
        self, member, rating=None, deviation=None, penality=0, reason=None
    ):
        ...
        await db.insert(
            ...
            dict(
                ...
                deviation_before=old["deviation"],
                rating_change=rating - old["rating"],
    ...
    async def hide_player(self, user_id, hide=True):
        await db.update(
            self.table,
            dict(is_hidden=hide),
            keys=dict(channel_id=self.channel_id, user_id=user_id),
        )
    ...
    async def snap_ratings(self, ranks_table):
        ...
        lowest = min(ranks)
        data = await db.select(
            ("*",), self.table, where=dict(channel_id=self.channel_id)
        )
        history = []
        now = int(time.time())
    ...
    async def reset(self):
        data = await db.select(
            ("user_id", "rating", "deviation"),
            self.table,
            where=dict(channel_id=self.channel_id),
        )
        history = []
        now = int(time.time())

        for p in data:
            if p["rating"] is not None and (
                p["rating"] != self.init_rp or p["deviation"] != self.init_deviation
            ):
                history.append(
                    dict(
                        user_id=p["user_id"],
        ...
        await db.update(
            self.table,
            dict(rating=None, deviation=None),
            keys=dict(channel_id=self.channel_id),
        )
        if len(history):
            await db.insert_many("qc_rating_history", history)
...
class Glicko2Rating(BaseRating):
    ...
    def rate(self, winners, losers, draw=False):
        ...
        avg_w = [
            ...
            [
                int(sum((p["deviation"] for p in winners)) / len(winners))
            ],  # average deviation
            [score_l],
        ]
        avg_l = [
            ...
            [
                int(sum((p["deviation"] for p in losers)) / len(losers))
            ],  # average deviation
            [score_w],
        ]
        ...
        for p in winners:
            ...
            new = self._scale_changes(
                p,
                po.getRating() - avg_w[0][0],
...
class TrueSkillRating(BaseRating):
    def __init__(self, **kwargs):
        ...
        self.ts = trueskill.TrueSkill(
            mu=self.init_rp,
            sigma=self.init_deviation,
            beta=int(self.init_deviation / 2),
            tau=int(self.init_deviation / 100),
        )

    def rate(self, winners, losers, draw=False):
        g1 = [
            self.ts.create_rating(mu=p["rating"], sigma=p["deviation"]) for p in winners
        ]
        g2 = [
            self.ts.create_rating(mu=p["rating"], sigma=p["deviation"]) for p in losers
        ]
        ...
        for p in winners:
            res = g1.pop(0)
            new = self._scale_changes(
                p, res.mu - p["rating"], res.sigma - p["deviation"], 0 if draw else 1
            )
            results.append(new)
