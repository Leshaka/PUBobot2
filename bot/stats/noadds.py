...
class NoAdds:
    ...
    async def get_user(qc, member):
        ...
        m_noadd = await db.select_one(
            ["duration", "at"],
            "noadds",
            where=dict(guild_id=qc.channel.guild.id, user_id=member.id, is_active=1),
        )
        ban_left = (
            max(0, (m_noadd["duration"] + m_noadd["at"]) - int(time.time()))
            if m_noadd
            else 0
        )
        phrases = await db.select(
            ["phrase"],
            "qc_phrases",
            where=dict(channel_id=qc.channel.id, user_id=member.id),
        )
    ...
    async def phrases_add(qc, member, phrase):
        await db.insert(
            "qc_phrases",
            dict(channel_id=qc.channel.id, user_id=member.id, phrase=phrase),
        )
    ...
    async def phrases_clear(qc, member=None):
        if member:
            await db.delete(
                "qc_phrases", where=dict(channel_id=qc.channel.id, user_id=member.id)
            )
        else:
    ...
    async def forgive(qc, member, moderator):
        ...
        if not noadd_id:
            return False
        await db.update(
            "noadds", dict(is_active=0, released_by=get_nick(moderator)), keys=noadd_id
        )
        return True
    ...
    async def get_noadds(qc):
        return await db.select(
            ["*"], "noadds", where=dict(guild_id=qc.channel.guild.id, is_active=1)
        )

    async def think(self, frame_time):
        if frame_time > self.next_tick:
            await db.execute(
                "UPDATE `noadds` SET is_active=0, released_by='time' WHERE (`at`+`duration`)<%s",
                (frame_time,),
            )
