...
async def enable_channel(message):
    if not (
        message.author.id == cfg.DC_OWNER_ID
    ...
    if message.channel.id not in queue_channels.keys():
        queue_channels[message.channel.id] = await bot.QueueChannel.create(
            message.channel
        )
...
async def disable_channel(message):
    ...
    if qc:
        ...
        await message.channel.send(
            embed=error_embed("The bot is not enabled on this channel.")
        )
