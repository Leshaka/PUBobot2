...
class CheckIn:
    ...
    def __init__(self, match, timeout):
        ...
        if len(self.m.cfg["maps"]) > 1 and self.m.cfg["vote_maps"]:
            self.maps = self.m.random_maps(
                self.m.cfg["maps"], self.m.cfg["vote_maps"], self.m.queue.last_map
            )
    ...
    async def start(self):
        ...
        emojis = (
            [self.READY_EMOJI, "🔸", self.NOT_READY_EMOJI]
            if self.allow_discard
            else [self.READY_EMOJI]
        )
        ...
        if len(not_ready):
            try:
                await self.message.edit(
                    content=None, embed=self.m.embeds.check_in(not_ready)
                )
            except DiscordException:
                pass
        else:
    ...
    async def set_ready(self, member, ready):
        if self.m.state != self.m.CHECK_IN:
            raise bot.Exc.MatchStateError(
                self.m.gt("The match is not on the check-in stage.")
            )
        if ready:
            ...
            if not self.allow_discard:
                raise bot.Exc.PermissionError(
                    self.m.gt("Discarding check-in is not allowed.")
                )
        ...
        await self.m.send(
            "\n".join(
                (
                    ...
                    self.m.gt("Reverting {queue} to the gathering stage...").format(
                        queue=f"**{self.m.queue.name}**"
                    ),
                )
            )
        )
    ...
    async def abort_timeout(self):
        ...
        await self.m.send(
            "\n".join(
                (
                    self.m.gt("{members} was not ready in time.").format(
                        members=join_and([m.mention for m in not_ready])
                    ),
                    self.m.gt("Reverting {queue} to the gathering stage...").format(
                        queue=f"**{self.m.queue.name}**"
                    ),
