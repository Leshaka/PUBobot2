...
class Match:
    ...
    default_cfg = dict(
        ...
        map_count=0,
        check_in_timeout=0,
        check_in_discard=True,
        match_lifetime=3 * 60 * 60,
        start_msg=None,
        server=None,
    )
    ...
    async def new(cls, queue, qc, players, **kwargs):
        ...
        ratings = {
            p["user_id"]: p["rating"]
            for p in await qc.rating.get_players((p.id for p in players))
        }
        bot.last_match_id += 1
        match = cls(bot.last_match_id, queue, qc, players, ratings, **kwargs)
        # Prepare the Match object
        match.maps = match.random_maps(
            match.cfg["maps"], match.cfg["map_count"], queue.last_map
        )
    ...
    async def from_json(cls, queue, qc, data):
        ...
        for i in range(len(data["teams"])):
            data["teams"][i] = [
                get(data["players"], id=user_id) for user_id in data["teams"][i]
            ]
        data["ready_players"] = [
            get(data["players"], id=user_id) for user_id in data["ready_players"]
        ]

        # Create the Match object
        ratings = {
            p["user_id"]: p["rating"]
            for p in await qc.rating.get_players((p.id for p in data["players"]))
        }
        bot.last_match_id += 1
        match = cls(
            bot.last_match_id, queue, qc, data["players"], ratings, **data["cfg"]
        )
    ...
    def __init__(self, match_id, queue, qc, players, ratings, **cfg):
        ...
        }  # filter kwargs for notnull values
    ...
    def sort_players(self, players):
        ...
        return sorted(
            players,
            key=lambda p: [
                self.cfg["captains_role_id"] in [role.id for role in p.roles],
                self.ratings[p.id],
            ],
            reverse=True,
        )
    ...
    def init_captains(self, pick_captains, captains_role_id):
        if pick_captains == "by role and rating":
            ...
            candidates = sorted(
                self.players, key=lambda p: [self.ratings[p.id]], reverse=True
            )
    ...
    def init_teams(self, pick_teams):
        if pick_teams == "draft":
            ...
            best_team = min(
                ...
                key=lambda team: abs(
                    sum([self.ratings[m.id] for m in team]) - best_rating
                ),
            )
            self.teams[0].set(self.sort_players(best_team[: self.cfg["team_size"]]))
            self.teams[1].set(
                self.sort_players(
                    [p for p in self.players if p not in best_team][
                        : self.cfg["team_size"]
                    ]
                )
            )
            ...
            self.teams[0].set(
                random.sample(
                    self.players, min(len(self.players) // 2, self.cfg["team_size"])
                )
            )
            self.teams[1].set(
                [p for p in self.players if p not in self.teams[0]][
                    : self.cfg["team_size"]
                ]
            )
    ...
    async def think(self, frame_time):
        if self.state == self.INIT:
            ...
            try:
                await self.qc.error(
                    self.gt("Match {queue} ({id}) has timed out.").format(
    ...
    async def report_loss(self, member, draw_flag):
        ...
        if team is None:
            raise bot.Exc.PermissionError(
                self.gt("You must be a team captain to report a loss or draw.")
            )
        ...
        if draw_flag and not enemy_team.draw_flag == draw_flag:
            ...
            await self.qc.channel.send(
                self.gt(
                    "{self} is calling a draw, waiting for {enemy} to type `{prefix}rd`."
                    if draw_flag == 1
                    else "{self} offers to cancel the match, waiting for {enemy} to type `{prefix}rc`."
                ).format(
                    self=member.mention,
    ...
    async def print_rating_results(self, before, after):
        ...
        if len(winners) == 1 and len(losers) == 1:
            ...
            for team in (winners, losers):
                ...
                msg += "\n".join(
                    (
                        f"> {get_nick(p)} {before[p.id]['rating']} ⟼ {after[p.id]['rating']}"
                        for p in team
                    )
                )
                n += 1
    ...
    async def cancel(self):
        if (
            ...
            and self.check_in.message.id in bot.waiting_reactions.keys()
        ):
