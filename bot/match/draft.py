...
class Draft:
    ...
    async def cap_for(self, author, team_name):
        if self.m.state != self.m.DRAFT:
            ...
            team := find(
                lambda t: t.name.lower() == team_name.lower(), self.m.teams[:2]
            )
        ) is None:
    ...
    async def pick(self, author, players):
        for player in players:
            ...
            picker_team = (
                self.m.teams[self.pick_order[pick_step]]
                if pick_step < len(self.pick_order) - 1
                else None
            )
            ...
            if self.m.state != self.m.DRAFT:
                ...
                raise bot.Exc.NotFoundError(
                    self.m.gt("Specified player not in the unpicked list.")
                )
            ...
            if len(self.m.teams[2]) and len(self.pick_order[pick_step + 1 :]) >= len(
                self.m.teams[2]
            ):
    ...
    async def put(self, player, team_name):
        if (
            team := find(lambda t: t.name.lower() == team_name.lower(), self.m.teams)
        ) is None:
            raise bot.Exc.SyntaxError(self.m.gt("Specified team name not found."))
        if self.m.state not in [self.m.DRAFT, self.m.WAITING_REPORT]:
            raise bot.Exc.MatchStateError(
                self.m.gt("The match must be on the draft or waiting report stage.")
            )
    ...
    async def sub_for(self, player1, player2, force=False):
        if self.m.state not in [self.m.CHECK_IN, self.m.DRAFT, self.m.WAITING_REPORT]:
            raise bot.Exc.MatchStateError(
                self.m.gt(
                    "The match must be on the check-in, draft or waiting report stage."
                )
            )
