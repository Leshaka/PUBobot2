...
class Embeds:
    ...
    def check_in(self, not_ready):
        ...
        if not len(self.m.check_in.maps):
            embed.add_field(
                name="—",
                value=self.m.gt(
                    "Please react with {ready_emoji} to **check-in** or {not_ready_emoji} to **abort**!"
                ).format(
            ...
            embed.add_field(
                ...
                value="\n".join(
                    [
                        self.m.gt(
                            "Please react with {ready_emoji} or vote for a map to **check-in**."
                        ).format(ready_emoji=self.m.check_in.READY_EMOJI),
    ...
    def draft(self):
        ...
        teams_names = [
            f"{t.emoji} \u200b **{t.name}**"
            + (
                f" \u200b `〈{sum((self.m.ratings[p.id] for p in t))//(len(t) or 1)}〉`"
                if self.m.ranked
                else ""
            )
        ...
        team_players = [
            " \u200b ".join(
                [
                    (f"`{self.m.rank_str(p)}" if self.m.ranked else "`")
                    + f"{get_nick(p)}`"
                    for p in t
                ]
            )
            if len(t)
            else self.m.gt("empty")
            for t in self.m.teams[:2]
        ]
        embed.add_field(
            name=teams_names[0],
            value=" \u200b ❲ \u200b " + team_players[0] + " \u200b ❳",
            inline=False,
        )
        embed.add_field(
            name=teams_names[1],
            value=" \u200b ❲ \u200b " + team_players[1] + " \u200b ❳\n\u200b",
            inline=False,
        )
        ...
        if len(self.m.teams[2]):
            ...
            if len(self.m.teams[0]) and len(self.m.teams[1]):
                msg = self.m.gt(
                    "Pick players with `{prefix}pick @player` command."
                ).format(prefix=self.m.qc.cfg.prefix)
                pick_step = len(self.m.teams[0]) + len(self.m.teams[1]) - 2
                picker_team = (
                    self.m.teams[self.m.draft.pick_order[pick_step]]
                    if pick_step < len(self.m.draft.pick_order) - 1
                    else None
                )
                if picker_team:
                    msg += "\n" + self.m.gt("{member}'s turn to pick!").format(
                        member=f"<@{picker_team[0].id}>"
                    )
            else:
                msg = self.m.gt(
    ...
    def final_message(self):
        ...
        if len(self.m.teams[0]) == 1 and len(self.m.teams[1]) == 1:  # 1v1
            ...
            if self.m.ranked or self.m.cfg["pick_captains"]:
                embed.add_field(
                    ...
                    value=" \u200b "
                    + join_and(
                        [self.m.teams[0][0].mention, self.m.teams[1][0].mention]
                    ),
                    inline=False,
                )
        ...
        if self.m.cfg["start_msg"]:
            embed.add_field(
                name="—", value=self.m.cfg["start_msg"] + "\n\u200b", inline=False
            )
        embed.set_footer(**self.footer)
