...
class ExpireTimer:
    ...
    def _define_next(self):
        if len(self.tasks):
            ...
            log.debug(
                f"EXPIRE TIMER NEXT > {self.next.member.name} ({self.next.qc.id}/{self.next.member.id})"
            )
        else:
            self.next = None
    ...
    def cancel(self, qc, member):
        ...
        if key in self.tasks.keys():
            ...
            log.debug(
                f"EXPIRE TIMER CANCEL > {task.member.name} ({task.qc.id}/{task.member.id})"
            )
    ...
    async def think(self, frame_time):
        if self.next and frame_time >= self.next.at:
            ...
            log.debug(
                f"EXPIRE TIMER TRIGGER > {task.member.name} ({task.qc.id}/{task.member.id})"
            )
            self._define_next()
            if task.qc and task.member:
                await task.qc.remove_members(
                    task.member, reason="expire", highlight=True
                )
