...
class PickupQueue:
    ...
    cfg_factory = CfgFactory(
        ...
        tables=[
            VariableTable(
                ...
                variables=[Variables.StrVar("alias", notnull=True)],
            ),
            VariableTable(
                "maps",
                display="Maps",
                ...
                variables=[Variables.StrVar("name", notnull=True)],
            ),
        ],
    )
    ...
    def validate_name(name):
        if not len(name) or any((c in name for c in ": \t\n")):
            raise ValueError(
                f"Invalid queue name '{name}'. A queue name should be one word without +-: characters."
            )
        return name
    ...
    async def from_json(self, data):
        players = [
            self.qc.channel.guild.get_member(user_id) for user_id in data["players"]
        ]
        if None in players:
            await self.qc.error(
                f"Unable to load queue **{self.cfg.name}**, error fetching guild members."
            )
            return
        self.queue = players
        if self.length:
    ...
    async def promote(self):
        ...
        promotion_msg = self.cfg.promotion_msg or self.qc.gt(
            "{role} Please add to **{name}** pickup, `{left}` players left!"
        )
        ...
        if (
            promotion_role
            and not promotion_role.mentionable
            and self.qc.channel.guild.me
            and not self.qc.channel.guild.me.guild_permissions.mention_everyone
        ):
            raise bot.Exc.PermissionError(
                "Insufficient permissions to ping the promotion role."
            )
        else:
    ...
    async def add_member(self, member):
        if (
            self.cfg.blacklist_role
            and self.cfg.blacklist_role in member.roles
            or self.cfg.whitelist_role
            and self.cfg.whitelist_role not in member.roles
        ):
    ...
    async def start(self):
        if len(self.queue) < 2:
            raise bot.Exc.PubobotException(
                self.qc.gt("Not enough players to start the queue.")
            )

        players = list(self.queue)
        dm_text = self.cfg.start_direct_msg or self.qc.gt(
            "**{queue}** pickup has started @ {channel}!"
        )
        await self.qc.queue_started(
            members=players,
            message=dm_text.format(
                queue=self.name, channel=self.qc.channel.mention, server=self.cfg.server
            ),
        )
        ...
        await bot.Match.new(
            ...
            team_emojis=self.cfg.team_emojis.split(" ")
            if self.cfg.team_emojis
            else None,
            ranked=self.cfg.ranked,
            team_size=int(self.cfg.size / 2),
            pick_captains=self.cfg.pick_captains,
            captains_role_id=self.cfg.captains_role.id
            if self.cfg.captains_role
            else None,
            pick_teams=self.cfg.pick_teams,
            pick_order=self.cfg.pick_order,
            maps=[i["name"] for i in self.cfg.tables.maps],
            vote_maps=self.cfg.vote_maps,
            map_count=self.cfg.map_count,
            check_in_timeout=self.cfg.check_in_timeout,
            check_in_discard=self.cfg.check_in_discard,
            start_msg=self.cfg.start_msg,
