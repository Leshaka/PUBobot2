...
class QueueChannel:
            ...
            VariableTable(
                ...
                variables=[
                    ...
                    Variables.IntVar(
                        "rating",
                        default=1200,
                        description="The rank will be given on this rating or higher.",
                    ),
                    Variables.RoleVar(
                        "role", description="Assign a guild role to the rank owners."
                    ),
                ],
                default=[
    ...
    async def create(cls, text_channel):
        ...
        for pq_cfg in await bot.PickupQueue.cfg_factory.select(
            text_channel.guild, {"channel_id": self.channel.id}
        ):
    ...
    async def apply_rating_decay(self):
        if self.id == self.rating.channel_id and (
            self.cfg.rating_decay or self.cfg.rating_deviation_decay
        ):
            await self.rating.apply_decay(
                self.cfg.rating_decay or 0,
                self.cfg.rating_deviation_decay or 0,
                self._ranks_table,
            )

    def access_level(self, member):
        if (
            self.cfg.admin_role in member.roles
            or member.id == cfg.DC_OWNER_ID
        ...
        if self.access_level(member) < req_perms:
            if req_perms == 2:
                raise bot.Exc.PermissionError(
                    self.gt("You must possess admin permissions.")
                )
            else:
                raise bot.Exc.PermissionError(
                    self.gt("You must possess moderator permissions.")
                )
    ...
    def _ranks_table(self):
        if self.cfg.rating_channel:
            return (
                bot.queue_channels.get(self.cfg.rating_channel.id) or self
            ).cfg.tables.ranks
        else:
    ...
    async def update_topic(self, force_announce=False, phrase=None):
        ...
        if not len(populated):
            ...
            new_topic = "\n".join(
                [f"> **{q.name}** ({q.status}) | {q.who}" for q in populated]
            )
        else:
            new_topic = (
                "> ["
                + " | ".join([f"**{q.name}** ({q.status})" for q in populated])
                + "]"
            )
            ...
            if reason:
                ...
                if len(affected) == 1:
                    await self.channel.send(
                        self.gt(
                            "{member} were removed from all queues ({reason})."
                        ).format(member=mention, reason=reason)
                    )
                else:
                    await self.channel.send(
                        self.gt(
                            "{members} were removed from all queues ({reason})."
                        ).format(members=mention, reason=reason)
                    )
    ...
    def get_member(self, string):
        if highlight := re.match(r"<@!?(\d+)>", string):
            ...
            string = string.lower()
            return find(
                lambda m: string == m.name.lower()
                or (m.nick and string == m.nick.lower()),
    ...
    async def get_lb(self):
        ...
        return [
            ...
            and not i["is_hidden"]
            and not (
                self.cfg.lb_min_matches
                and self.cfg.lb_min_matches > sum((i["wins"], i["losses"], i["draws"]))
            )
        ]
    ...
    async def update_expire(self, member):
        ...
        personal_expire = await db.select_one(
            ["expire"], "players", where={"user_id": member.id}
        )
    ...
    async def _update_rating_roles(self, *members):
        ...
        for member in members:
            to_delete = [
                ...
                if role != roles[member.id] and role in member.roles
            ]
            try:
                if len(to_delete):
                    await member.remove_roles(*to_delete, reason="Rank update.")
                if (
                    roles[member.id] is not None
                    and roles[member.id] not in member.roles
                ):
    ...
    async def _dm_members(self, members, *args, **kwargs):
        for m in members:
            if not m.bot and not await db.select_one(
                ("user_id",), "players", where={"user_id": m.id, "allow_dm": 0}
            ):
                try:
    ...
    async def process_msg(self, message):
        ...
        # normal commands starting with prefix
            ...
            if await db.select_one(
                ["guild_id"],
                "disabled_guilds",
                where={"guild_id": self.channel.guild.id},
            ):
                await self.error(
        ...
        if f:
            ...
            try:
                ...
                await message.channel.send(
                    embed=error_embed(str(e), title=type(e).__name__)
                )
            except BaseException as e:
                await message.channel.send(
                    embed=error_embed(str(e), title="RuntimeError")
                )
                log.error(
                    f"Error processing last message. Traceback:\n{traceback.format_exc()}======"
                )
    ...
    async def _create_pickup(self, message, args=""):
        ...
        if len(args) != 2 or not args[1].isdigit():
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}create_pickup __name__ __size__"
            )
        try:
    ...
    async def _delete_queue(self, message, args=None):
        ...
        if (queue := get(self.queues, name=args)) is None:
            raise bot.Exc.NotFoundError(
                f"Specified queue name not found on the channel."
            )
    ...
    async def _show_queues(self, message, args=None):
        if len(self.queues):
            await self.channel.send(
                "> ["
                + " | ".join([f"**{q.name}** ({q.status})" for q in self.queues])
                + "]"
            )
        else:
    ...
    async def _add_member(self, message, args=None):
        if self.cfg.blacklist_role and self.cfg.blacklist_role in message.author.roles:
            raise bot.Exc.PermissionError(
                self.gt("You are not allowed to add to queues on this channel.")
            )
        if (
            self.cfg.whitelist_role
            and self.cfg.whitelist_role not in message.author.roles
        ):
            raise bot.Exc.PermissionError(
                self.gt("You are not allowed to add to queues on this channel.")
            )
        ...
        if ban_left:
            raise bot.Exc.PermissionError(
                self.gt("You have been banned, `{duration}` left.").format(
        ...
        # select queues requested by user
            ...
            t_queues = [
                ...
                if any(
                    (
                        t == q.name.lower()
                        or t in (a["alias"].lower() for a in q.cfg.tables.aliases)
                        for t in targets
                    )
                )
            ]
        ...
        if len(not_allowed := [q for q in qr.keys() if qr[q] == bot.Qr.NotAllowed]):
            await self.error(
                self.gt(
                    "You are not allowed to add to {queues} queues.".format(
        ...
        if bot.Qr.Success in qr.values():
            ...
            await self.update_topic(
                phrase=f"{message.author.mention}, {phrase}" if phrase else None
            )
    ...
    async def _set(self, message, args=""):
        ...
        if len(args) != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}set __variable__ __value__"
            )
    ...
    async def _set_queue(self, message, args=""):
        ...
        if len(args) != 3:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}set_queue __queue__ __variable__ __value__"
            )
        if (
    ...
    async def _set_cfg_queue(self, message, args=""):
        ...
        if len(args) != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}set_cfg_queue __queue__ __json__"
            )
    ...
    async def _put(self, message, args=""):
        ...
        if len(args) < 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}put __player__ __team__"
            )
    ...
    async def _sub_force(self, message, args=""):
        ...
        if len(args := args.split(" ")) != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}sub_force __@player1__ __@player2__"
            )
    ...
    async def _rank(self, message, args=None):
        ...
        if p:
            ...
            embed.add_field(
                name=self.gt("Matches"),
                value=f"**{(p['wins']+p['losses']+p['draws'])}**",
                inline=True,
            )
            if p["rating"]:
                embed.add_field(
                    name=self.gt("Rank"),
                    value=f"**{self.rating_rank(p['rating'])['rank']}**",
                    inline=True,
                )
                embed.add_field(
                    name=self.gt("Rating"), value=f"**{p['rating']}**±{p['deviation']}"
                )
            else:
            ...
            changes = await db.select(
                ...
                "qc_rating_history",
                where=dict(user_id=member.id, channel_id=self.rating.channel_id),
                order_by="id",
                limit=5,
            )
            if len(changes):
                    ...
                    value="\n".join(
                        (
                            "\u200b \u200b **{change}** \u200b | {ago} ago | {reason}{match_id}".format(
                                ...
                                reason=c["reason"],
                                match_id=f"(__{c['match_id']}__)"
                                if c["match_id"]
                                else "",
                                change=("+" if c["rating_change"] >= 0 else "")
                                + str(c["rating_change"]),
                            )
                            for c in changes
                        )
    ...
    async def _rw(self, message, args=""):
        ...
        if len(args) != 2 or not args[0].isdigit():
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}rw __match_id__ __team_name__ or draw"
            )
        if (
            match := find(
                lambda m: m.qc == self and m.id == int(args[0]), bot.active_matches
            )
        ) is None:
            raise bot.Exc.NotFoundError(
                self.gt(
                    "Could not find match with specified id. Check `{prefix}matches`."
                ).format(prefix=self.cfg.prefix)
            )
    ...
    async def _expire(self, message, args=None):
        if not args:
            if task := bot.expire.get(self, message.author):
                await self.channel.send(
                    self.gt("You have {duration} expire time left.").format(
                        duration=seconds_to_str(task.at - int(time.time()))
                    )
                )
            else:
                await self.channel.send(
                    self.gt("You don't have an expire timer set right now.")
                )
            ...
            try:
                ...
                raise bot.Exc.SyntaxError(
                    self.gt("Invalid duration format. Syntax: 3h2m1s or 03:02:01.")
                )

            if secs > MAX_EXPIRE_TIME:
                raise bot.Exc.ValueError(
                    self.gt(
                        "Expire time must be less than {time}.".format(
            ...
            await self.success(
                self.gt("Set your expire time to {duration}.").format(
    ...
    async def _default_expire(self, message, args=None):
        if not args:
            data = await db.select_one(
                ["expire"], "players", where={"user_id": message.author.id}
            )
            ...
            if args == "afk":
                ...
                try:
                    ...
                    raise bot.Exc.SyntaxError(
                        self.gt(
                            "Invalid duration format. Syntax: 3h2m1s or 03:02:01 or AFK."
                        )
                    )
        ...
        if expire == 0:
            ...
            text = self.gt(
                "Your default expire time is {time}.".format(
                    time=seconds_to_str(expire)
                )
            )
        ...
        if not modify:
            ...
            try:
                await db.insert(
                    "players", {"user_id": message.author.id, "expire": expire}
                )
            except db.errors.IntegrityError:
                await db.update(
                    "players", {"expire": expire}, keys={"user_id": message.author.id}
                )
    ...
    async def _allow_offline(self, message, args=None):
        if message.author.id in bot.allow_offline:
            ...
            await self.success(
                self.gt("Your offline immunity is **on** until the next match.")
            )
                ...
                discord_table(
                    ...
                    [
                        [
                            (page * 10) + (n + 1),
                            str(data[n]["rating"])
                            + self.rating_rank(data[n]["rating"])["rank"],
                            data[n]["nick"].strip(),
                            ...
                            "{0}/{1}/{2} ({3}%)".format(
                                ...
                                int(
                                    data[n]["wins"]
                                    * 100
                                    / ((data[n]["wins"] + data[n]["losses"]) or 1)
                                ),
                            ),
                        ]
    ...
    async def _promote(self, message, args=None):
        if not args:
            if (
                queue := next(
                    iter(
                        sorted(
                            (q for q in self.queues if q.length),
            ...
            if (
                queue := find(lambda q: q.name.lower() == args.lower(), self.queues)
            ) is None:
        ...
        if (
            ...
            and self.cfg.promotion_delay + self.last_promote > now
        ):
            raise bot.Exc.PermissionError(
                self.gt(
                    "You're promoting too often, please wait `{delay}` until next promote.".format(
                        delay=seconds_to_str(
                            (self.cfg.promotion_delay + self.last_promote) - now
                        )
                    )
                )
            )
    ...
    async def _rating_set(self, message, args=None):
        ...
        try:
            ...
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}rating_set __@user__ __rating__ [__deviation__]"
            )

        if not 0 < rating < 10000 or not 0 < (deviation or 1) < 3000:
            raise bot.Exc.ValueError("Bad rating or deviation value.")

        await self.rating.set_rating(
            member, rating=rating, deviation=deviation, reason="manual seeding"
        )
    ...
    async def _rating_penality(self, message, args=None):
        ...
        try:
            ...
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}rating_penality __@user__ __points__ [__reason__]"
            )
    ...
    async def _rating_unhide(self, message, args=None):
        ...
        if (member := self.get_member(args)) is None:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}rating_unhide __@user__"
            )
    ...
    async def _cancel_match(self, message, args=""):
        ...
        if not args.isdigit():
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}cancel_match __match_id__"
            )
        ...
        if match.qc != self:
            raise bot.Exc.PermissionError(
                "Specified match does not belong to this channel."
            )
    ...
    async def _undo_match(self, message, args=""):
        ...
        if not args.isdigit():
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}undo_match __match_id__"
            )
    ...
    async def _switch_dms(self, message, args=""):
        data = await db.select_one(
            ("allow_dm",), "players", where={"user_id": message.author.id}
        )
        if data:
            allow_dm = 1 if data["allow_dm"] == 0 else 0
            await db.update(
                "players", {"allow_dm": allow_dm}, keys={"user_id": message.author.id}
            )
        else:
            allow_dm = 0
            await db.insert(
                "players", {"allow_dm": allow_dm, "user_id": message.author.id}
            )

        if allow_dm:
            await self.success(
                self.gt("Your DM notifications is now turned on."),
                reply_to=message.author,
            )
        else:
            await self.success(
                self.gt("Your DM notifications is now turned off."),
                reply_to=message.author,
            )
    ...
    async def _stats_reset_player(self, message, args=None):
        ...
        if (member := self.get_member(args)) is None:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}stats_reset_player __@user__"
            )
    ...
    async def _stats_replace_player(self, message, args=""):
        ...
        if (args := args.split(" ")).__len__() != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}stats_reset_player __@user1__ __@user2__"
            )
        if None in (members := [self.get_member(string) for string in args]):
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}stats_reset_player __@user1__ __@user2__"
            )
        await bot.stats.replace_player(
            self.channel.id, members[0].id, members[1].id, get_nick(members[1])
        )
        await self.success(self.gt("Done."))
    ...
    async def _last_game(self, message, args=None):
        if args:
            if queue := find(lambda q: q.name.lower() == args.lower(), self.queues):
                last_game = await db.select_one(
                    ["*"],
                    "qc_matches",
                    where=dict(channel_id=self.id, queue_id=queue.id),
                    order_by="match_id",
                    limit=1,
                )
                ...
                if match := await db.select_one(
                    ["match_id"],
                    "qc_player_matches",
                    where=dict(channel_id=self.id, user_id=member.id),
                    order_by="match_id",
                    limit=1,
                ):
                    last_game = await db.select_one(
                        ["*"],
                        "qc_matches",
                        where=dict(channel_id=self.id, match_id=match["match_id"]),
                    )
                else:
                    last_game = None
            else:
                raise bot.Exc.SyntaxError(
                    f"Usage: {self.cfg.prefix}lastgame [__queue__]"
                )
        else:
            last_game = await db.select_one(
                ["*"],
                "qc_matches",
                where=dict(channel_id=self.id),
                order_by="match_id",
                limit=1,
            )
        ...
        players = await db.select(
            ["user_id", "nick", "team"],
            "qc_player_matches",
            where=dict(match_id=last_game["match_id"]),
        )
        embed = Embed(colour=Colour(0x50E3C2))
        embed.add_field(
            name=last_game["queue_name"],
            value=seconds_to_str(int(time.time()) - last_game["at"]) + " ago",
        )
        if len(team := [p["nick"] for p in players if p["team"] == 0]):
            embed.add_field(
                name=last_game["alpha_name"], value="`" + ", ".join(team) + "`"
            )
        if len(team := [p["nick"] for p in players if p["team"] == 1]):
            embed.add_field(
                name=last_game["beta_name"], value="`" + ", ".join(team) + "`"
            )
    ...
    async def _remove_player(self, message, args=None):
        ...
        if not args:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}remove_player __@user__"
            )
        elif (member := self.get_member(args)) is None:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}remove_player __@user__"
    ...
    async def _add_player(self, message, args=""):
        ...
        if len(args) != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}add_player __queue__ __@user__"
            )
        elif (
            queue := find(lambda q: q.name.lower() == args[0].lower(), self.queues)
        ) is None:
    ...
    async def subscribe(self, member, args, unsub=False):
        if not args:
            ...
            args = args.split(" ")
            roles = (
                q.cfg.promotion_role
                for q in self.queues
                if q.cfg.promotion_role
                and any(
                    (
                        t == q.name.lower()
                        or t in (a["alias"].lower() for a in q.cfg.tables.aliases)
        ...
        if unsub:
            ...
            await self.success(
                self.gt("Removed `{count}` roles from you.").format(count=len(roles))
            )
            ...
            await self.success(
                self.gt("Added `{count}` roles to you.").format(count=len(roles))
            )
    ...
    async def _server(self, message, args=""):
        ...
        if not q.cfg.server:
            raise bot.Exc.NotFoundError(
                self.gt("Server for **{queue}** is not set.").format(queue=q.name)
            )
        await self.success(
            q.cfg.server, title=self.gt("Server for **{queue}**").format(queue=q.name)
        )
    ...
    async def _stats(self, message, args=None):
        ...
        embed = Embed(
            ...
            description=self.gt("**Total matches: {count}**").format(
                count=stats["total"]
            ),
        )
    ...
    async def _top(self, message, args=None):
        ...
        if args in ["day", self.gt("day")]:
            ...
            raise bot.Exc.SyntaxError(
                "Usage: {prefix}top [{options}]".format(
                    prefix=self.cfg.prefix,
                    options=" / ".join(
                        (self.gt(i) for i in ("day", "week", "month", "year"))
                    ),
                )
            )
        else:
        ...
        embed = Embed(
            title=self.gt("Top 10 players for __{target}__").format(
                target=f"#{self.channel.name}"
            ),
            colour=Colour(0x50E3C2),
            description=self.gt("**Total matches: {count}**").format(
    ...
    async def _cointoss(self, message, args=None):
        if not args or args.lower() in ["heads", self.gt("heads")]:
            ...
            raise bot.Exc.SyntaxError(
                "Usage: {prefix}ct [{options}]".format(
                    prefix=self.cfg.prefix,
        ...
        if pick == result:
            await self.channel.send(
                self.gt("{member} won, its **{side}**!").format(
                    member=message.author.mention,
                    side=self.gt(["heads", "tails"][result]),
                )
            )
        else:
            await self.channel.send(
                self.gt("{member} lost, its **{side}**!").format(
                    member=message.author.mention,
                    side=self.gt(["heads", "tails"][result]),
                )
            )
    ...
    async def maps(self, message, args="", random=False):
        ...
        if not len(q.cfg.tables.maps):
            raise bot.Exc.NotFoundError(
                self.gt("No maps is set for **{queue}**.").format(queue=q.name)
            )
    ...
    async def _noadd(self, message, args=""):
        ...
        if len(args := args.split(" ", maxsplit=2)) < 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}noadd __@user__ __duration__ [__reason__]"
            )
        ...
        await self.success(
            self.gt("Banned **{member}** for `{duration}`.").format(
                member=get_nick(member), duration=seconds_to_str(secs)
            )
        )
    ...
    async def _phrases_add(self, message, args=""):
        ...
        if len(args := args.split(" ", maxsplit=1)) != 2:
            raise bot.Exc.SyntaxError(
                f"Usage: {self.cfg.prefix}phrases_add __@user__ __phrase__"
            )
        elif (member := self.get_member(args.pop(0))) is None:
            raise bot.Exc.SyntaxError(self.gt("Specified user not found."))
