...
async def on_reaction_remove(
    reaction, user
):  # FIXME: this event does not get triggered for some reason
    if (
        user.id != dc.user.id
        and reaction.message.channel.id in bot.waiting_reactions.keys()
    ):
...
@dc.event
async def on_ready():
    await dc.change_presence(
        activity=Activity(type=ActivityType.watching, name=cfg.STATUS)
    )
    if not dc.was_ready:  # Connected for the first time, load everything
        ...
        for channel_id in await bot.QueueChannel.cfg_factory.p_keys():
            ...
            if channel:
                ...
                log.info(
                    f"\tInit channel {channel.guild.name}>#{channel.name} successful."
                )
            else:
        ...
        for qc in list(bot.queue_channels.values()):
            if channel := dc.get_channel(qc.id) is not None:
                ...
                log.error(
                    "ERROR! Channel missing after reconnect {}>#{} ({})!".format(
                        qc.cfg.cfg_info.get("guild_name"),
                        qc.cfg.cfg_info.get("channel_name"),
                        qc.id,
                    )
                )
...
async def on_member_update(before, after):
    if str(after.status) in ["idle", "offline"]:
        if after.id not in bot.allow_offline:
            for qc in filter(
                lambda i: i.channel and i.channel.guild.id == after.guild.id,
                bot.queue_channels.values(),
            ):
...
async def on_member_remove(member):
    for qc in filter(
        lambda i: i.channel and i.channel.guild.id == member.guild.id,
        bot.queue_channels.values(),
    ):
