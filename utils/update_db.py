...
for chan in channels:
    ...
    team_names = (
        chan["team_names"]
        if chan["team_names"] and len(chan["team_names"]) == 2
        else "Alpha Beta"
    )
    new_channel = {
    ...
    c1.execute(
        "SELECT * FROM pickup_configs WHERE channel_id = ?", (chan["channel_id"],)
    )
    ...
    for queue in old_queues:
        ...
        q_team_names = (
            q_team_names
            if q_team_names and len(q_team_names.split(" ")) == 2
            else "Alpha Beta"
        )
        q_team_emojis = get_value(queue, chan, "team_emojis")
        q_team_emojis = (
            q_team_emojis
            if q_team_emojis and len(q_team_emojis.split(" ")) == 2
            else None
        )
        ...
        if q_start_msg:
            ...
            q_start_msg = q_start_msg.replace(
                "%password%", get_value(queue, chan, "password") or ""
            )
        new_queue = {
            ...
            "is_default": 1
            if chan["++_req_players"] and chan["++_req_players"] <= queue["maxplayers"]
            else 0,
    ...
    c1.execute(
        "SELECT * FROM channel_players WHERE channel_id = ?", (chan["channel_id"],)
    )
    old_players = c1.fetchall()
    for player in old_players:
        rating = max(player["rank"], 0) if player["rank"] else player["rank"]
        deviation = 300 - (
            10 * (min(15, int(player["wins"] or 0) + int(player["loses"] or 0)))
        )
        new_player = {
    ...
    c1.execute(
        "SELECT * FROM player_pickups WHERE channel_id = ?", (chan["channel_id"],)
    )
