...
fkey_blank = dict(
    cname=None, refTable=None, refColumn=None, on_delete=None, on_update=None
)
...
class Adapter:
    ...
    def _mysql_column(kwargs):
        return "`{cname}` {ctype}{notnull}{unique}{autoincrement}{default}".format(
            ...
            default=" DEFAULT '{}'".format(kwargs["default"])
    ...
    def _mysql_fkey(kwargs):
        return (
            "({cname}) REFERENCES {refTable}({refColumn}){on_delete}{on_update}".format(
                cname=kwargs["cname"],
                refTable=kwargs["refTable"],
                refColumn=kwargs["refColumn"],
                on_delete=" ON DELETE " + reference_options[kwargs["on_delete"]]
                if kwargs["on_delete"]
                else "",
                on_update=" ON UPDATE " + reference_options[kwargs["on_update"]]
                if kwargs["on_update"]
                else "",
            )
        )
    ...
    async def create_table(self, table):
        ...
        columns = [
            self._mysql_column({**column_blank, **col}) for col in table["columns"]
        ]
        fkeys = [
            "FOREIGN KEY " + self._mysql_fkey({**fkey_blank, **fkey})
        ...
        request = "CREATE TABLE {tname} ({tdeskr})".format(
            tname=table["tname"], tdeskr=", ".join((columns + fkeys)) + pkeys
        )
    ...
    async def _ensure_table(self, table):
        ...
        for col in table["columns"]:
            ...
            if col["cname"] not in columns.keys():
                await self.execute(
                    "ALTER TABLE {tname} ADD COLUMN {column_sql}".format(
                        tname=table["tname"],
                ...
                for fkey in (
                    fkey
                    for fkey in table["foreign_keys"]
                    if fkey["cname"] == col["cname"]
                ):
                    await self.execute(
                        "ALTER TABLE {tname} ADD FOREIGN KEY {fkey_sql}".format(
                            tname=table["tname"],
                ...
                raise (
                    TypeError(
                        "Column '{}' types are mismatching, {} and {}".format(
                            col["cname"], col["ctype"], columns[col["cname"]]
                        )
                    )
                )
    ...
    async def select(
        ...
        conditions = (
            " WHERE " + " AND ".join(("`{}`=%s".format(k) for k in where.keys()))
            if where
            else ""
        )
