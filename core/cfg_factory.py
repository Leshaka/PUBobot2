        ...
        db.ensure_table(
            dict(
                ...
                columns=[
                    ...
                    *(
                        dict(cname=v.name, ctype=v.ctype, default=v.default)
                        for v in variables
                    ),
                ],
            ...
            db.ensure_table(
                dict(
                    ...
                    columns=[
                        ...
                        *(
                            dict(cname=v.name, ctype=v.ctype, default=v.default)
                            for v in var_table.variables.values()
                        ),
                    ],
                )
            )
            ...
            for var_table in self.tables.values():
                if var_table.default:
                    await db.insert_many(
                        var_table.table,
                        [{self.p_key: p_key, **row} for row in var_table.default],
                    )
...
class Config:
    ...
    async def load(cls, cfg_factory, row, guild):
        ...
        for table in self._factory.tables.values():
            ...
            for row in await db.select(
                table.variables.keys(), table.table, {self._factory.p_key: self.p_key}
            ):
                try:
                    objects.append(await table.wrap_row(row, self._guild))
                except Exception as e:
                    log.error(
                        "Failed to wrap a table row '{}' from '{}': {}".format(
                            row, table.name, str(e)
                        )
                    )
    ...
    async def update(self, data):
        ...
        if len(data):
            ...
            await db.update(
                self._factory.table, data, {self._factory.p_key: self.p_key}
            )
    ...
    def to_json(self):
        data = {
            key: value.readable(getattr(self, key))
            for key, value in self._factory.variables.items()
        }
        data["tables"] = dict()
    ...
    async def set_info(self, d):
        ...
        await db.update(
            self._factory.table,
            {"cfg_info": json.dumps(d)},
            {self._factory.p_key: self.p_key},
        )
...
class EmojiVar(Variable):
    ...
    async def validate(self, string, guild):
        ...
        if re.match("^:[^ ]*:$", string):
            return format_emoji(string.strip(":"), guild) or emoji.emojize(
                string, use_aliases=True
            )
        else:
            return string
            ...
            try:
                role_id = next(
                    (
                        ...
                        if role.name == string or str(role.id) == string
                    )
                ).id
    ...
    async def wrap(self, value, guild):
        if value:
            ...
            if role:
                ...
                raise ValueError(
                    "Requested role for variable '{}' not found on the guild".format(
                        self.name
                    )
                )
            ...
            try:
                user_id = next(
                    (
                        ...
                        if (member.nick or member.name).lower() == string
                        or str(member.id) == string
                    )
                )
            except StopIteration:
                raise ValueError("User '{}' not found on the guild.".format(string))
    ...
    async def wrap(self, value, guild):
        if value:
            ...
            if member:
                ...
                raise ValueError(
                    "Requested user for variable '{}' not found on the guild.".format(
                        self.name
                    )
                )
    ...
    async def validate(self, string, guild):
        ...
        if mention:
            ...
            if not guild.get_channel(channel_id):
                raise ValueError(
                    "Channel '{}' not found on the dc guild.".format(string)
                )
            ...
            try:
                channel_id = next(
                    (
                        ...
                        if channel.name == string.lstrip("#")
                        or str(channel.id) == string
                    )
                ).id
            except StopIteration:
    ...
    async def wrap(self, value, guild):
        if value:
            ...
            if channel:
                ...
                raise ValueError(
                    "Requested channel for variable '{}' not found on the guild.".format(
                        self.name
                    )
                )
...
class VariableTable:
    ...
    async def validate(self, data, guild):
        ...
        for row in data:
            ...
            validated.append(
                {
                    var_name: await self.variables[var_name].validate(value, guild)
    ...
    async def wrap(self, data, guild):
        ...
        for row in data:
            wrapped.append(
                {
                    var_name: await self.variables[var_name].wrap(value, guild)
                    for var_name, value in row.items()
                }
            )
        return wrapped
    ...
    def readable(self, l):
        return [
            {
                var_name: self.variables[var_name].readable(value)
                for var_name, value in d.items()
            }
            for d in l
        ]
    ...
    def verify(self, l):
        return all(
            (
                all((self.variables[key].verify(value) for key, value in d.items()))
                for d in l
            )
        )
