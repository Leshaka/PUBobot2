...
def format_message(_string, _guild, **kwargs):
    _string = re.sub(
        "#([^ ,.!?]+)",
        lambda i: format_channel(i.group(1), _guild) or i.group(0),
        _string,
    )
    _string = re.sub(
        "@([^ ,.!?]+)", lambda i: format_role(i.group(1), _guild) or i.group(0), _string
    )
    _string = re.sub(
        ":([^ ,.!?]+):",
        lambda i: format_emoji(i.group(1), _guild) or i.group(0),
        _string,
    )
